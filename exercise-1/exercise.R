...
# What do you see?
ggplot(data = diamonds.sample) # an empty square/canvas
...
ggplot(data = diamonds.sample, aes(carat, price)) +
  geom_point(aes(shape = factor(cut)))
...
ggplot(data = diamonds.sample, aes(carat, price)) +
  geom_point(aes(size = price, color = price))
...
ggplot(data = diamonds.sample, aes(carat, price)) +
  geom_smooth(aes(color = carat), se = FALSE)
...
ggplot(data = diamonds.sample) +
  geom_bar(mapping = aes(x = cut, fill = clarity)) # bar chart of stacked bars
